...
import { AIService } from '@replyguy/ai';
...
export class UserService {
    ...
    private async subscribeExistingUser(fid: string, alreadySubscribedFIDs: string[]) {
    ...
    private async registerNewUser(fid: string, alreadySubscribedFIDs: string[]) {
        ...
        const summary = await this.db.summarizeUserContextSafe(userData);
        if (!summary) {
