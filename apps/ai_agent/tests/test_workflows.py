...
@pytest.mark.asyncio
async def test_user_summary_workflow(mock_user_data, mock_user_casts, real_openai_config):
...
@pytest.mark.asyncio
async def test_trending_galaxy_workflow(mock_trending_casts, real_openai_config):
...
@pytest.mark.asyncio
async def test_integrated_workflow(mock_user_data, mock_user_casts, mock_trending_casts, real_openai_config):
    ...
    trending_result = await trending_workflow.run({
        "casts": mock_trending_casts["casts"],  # Pass just the casts list
        "user_embedding": user_result["user_embedding"]["vector"],  # Pass embedding at top level
    ...
    reply_result = await reply_workflow.process({
        ...
        "available_feeds": trending_result["trending_clusters"][:2]  # Use top 2 clusters
    })
